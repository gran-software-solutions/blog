...
`Last-Modified` <u>[HTTP response header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Last-Modified)</u> is another way to handle caching/concurrency. How does it work? When we fetch a resource from the server, we get a `Last-Modified` header in the response. This header contains the **date
